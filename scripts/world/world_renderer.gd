...
func _build_ocean() -> void:
	...
	sm.set_shader_parameter("water_alpha",           0.99)
	sm.set_shader_parameter("fresnel_sky_mix",       0.48)
	sm.set_shader_parameter("fresnel_power",         3.1)
	...
	sm.set_shader_parameter("roughness",             0.20)
	sm.set_shader_parameter("metallic",             0.0)
	sm.set_shader_parameter("specular",             0.40)
...
func _apply_ocean_shader(daylight: float, cloud: float, rain: float, wind: float, storm: float, fog_t: float) -> void:
	...
	_ocean_shader_material.set_shader_parameter("water_alpha",            lerpf(0.97, 1.0, rain))
